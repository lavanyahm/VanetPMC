...
void    AnimationInterface::trace(uint32_t src,uint32_t dst,double txt,double rxt)
{
  int auid = (++gAnimUid);
  AnimXmlElement element ("pr");
  element.AddAttribute ("uId", auid);
  element.AddAttribute ("fId", src);
  element.AddAttribute ("fbTx", txt);
  WriteN (element.ToString (),  m_f);

  WriteXmlP (auid, "wpr", dst, rxt, 0);
}
